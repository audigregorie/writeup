  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      if (!newUser.displayName) {
        await updateProfile(newUser, { displayName: 'New User' });
      }
    ...
    <>
      {modal && (
        <Modal modal={modal} setModal={setModal}>
          <section className={`shadows fixed z-50 overflow-auto bg-white px-12 pb-28 transition-all duration-200`}>
            ...
            <div className="flex flex-col items-center justify-center gap-12">
              {signRequest === '' ? (
                ...
                <SignIn setModal={setModal} setSignRequest={setSignRequest} />
              ) : signRequest === 'sign-up' ? (
